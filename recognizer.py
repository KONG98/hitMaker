...
def split(chunk, min_silence_len, length_limit, silence_thresh, level=0):
    ...
    if len(chunk) > length_limit:
        ...
        (chunk_splits, NonSilencePair) = split_on_silence(chunk, min_silence_len=min_silence_len,
...
def wavToPcm(wavName):
    # wav -> pcm
    getpcm = 'ffmpeg -i ./chunks/%s -ar 16000 -ac 1 -acodec pcm_s16le -f s16le ./chunks/%s.pcm' % (
    wavName, wavName[:-4])
